...
#############################FLICKING OUTPUTS###########################
    ...
    if(difference_gyro_Z > 20 and just_flicked_right_flag == 0):
        just_flicked_left_flag = 1
        counter_flicked_left = 1
        outputString += "\tLEFT FLICK DETECTED!"
    if(difference_gyro_Z < -20 and just_flicked_left_flag == 0):
        just_flicked_right_flag = 1
        counter_flicked_right = 1
        outputString += "\tRIGHT FLICK DETECTED!"

    if(difference_gyro_X > 40 and just_flicked_up_flag == 0): #bigger numbers so that it's not as sensitive
        just_flicked_down_flag = 1
        counter_flicked_down = 1
        outputString += "\tFLICK DOWN DETECTED"
    if(difference_gyro_X < -40 and just_flicked_down_flag == 0):
